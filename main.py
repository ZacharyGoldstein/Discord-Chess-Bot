...
with open('/Users/austinliu/Downloads/Discord-Chess-Bot-master/secrets.json') as fin:
...
async def whenrating(ctx, name, rating, variant="Rapid"):
  if not rating.isnumeric():
    ...
    return
  (prob_success,predicted_date) = score(name,int(rating),variant)
...
# except:
  if predicted_date == None:
    ...
    return
  await ctx.send(f'{name} can expect to have a {variant} rating of {rating} on {predicted_date.strftime("%b %d, %Y")}.')
